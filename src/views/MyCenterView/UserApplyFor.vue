...
<script>
	...
	methods: {
		...
		getApplyForInfoFunc(id) {
			getApplyForInfo({
				...
				if (response.StatusCode == 200) {
					...
					setTimeout(() => {
						// this.worker = this.$worker.run(() => this.getPdf(),[])
						// 	.then(res => console.log(res),this.btn_loading = '-1')
						// 	.catch(e => console.log(e,row.Code),this.btn_loading = -1)
						// this.destroyed()
						this.getPdf()
						this.btn_loading = -1
					}, 800)
		...
		webWorksFunc(){
			this.worker = this.$worker.run(n => n + 10, [4])
			.then(res => console.log(res))
			.catch(e => console.log(e)) }, destroyed() {
			// 通过赋值null的方式，释放掉worker引用，关闭worker
			this.worker = null
		},
